...
class MovieHandler(IMovieHandler):
    ...
    def add_to_library(self, update: Update, context: CallbackContext):
        ...
        if not context.user_data.get("quality_profile") and query.data.startswith("MovieQuality"):
            context.user_data["quality_profile"] = query.data.removeprefix("MovieQuality: ")
